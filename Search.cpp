	...
	while (j < 15)
	{
		if (*((*(puzzleArray + i)) + j) == *((word->word) + index)) // If a matching letter is found.
		{
			...
			if (length == strlen(word->word)) // If the word is found.
			{
				...
				colend = j;
				writeFoundWord(word->id, "VERTICAL", rowbeg, rowend, colbeg, colend); // Write to file.
				return true;
			}
	...
	while (i < 15) 
	{
		if (*((*(puzzleArray + i)) + j) == *((word->word) + index)) // If a matching letter is found.
		{
			...
			if (length == strlen(word->word)) // If the word is found.
			{
				rowend = i; // Keep the ending pos. of the word.
				colend = j;
				writeFoundWord(word->id, "HORIZONTAL", rowbeg, rowend, colbeg, colend); // Write to file.
				return true;
			}
	...
	while (i < 15)
	{
		if (*((*(puzzleArray + i)) + j) == *((word->word) + index)) // If a matching letter is found.
		{
			...
			if (length == strlen(word->word)) // If the word is found.
			{
				rowend = i; // Keep the ending pos. of the word.
				colend = j;
				writeFoundWord(word->id, "DIAGONAL", rowbeg, rowend, colbeg, colend); // Write to file.
				return true;
			}
